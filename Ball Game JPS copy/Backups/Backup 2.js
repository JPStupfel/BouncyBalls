...
function makeCoords(x) {
    for (let i = 0; i < x; i++){
        ...
        defaultCoords[`Circle${i}`]["radius"] = getRandomIntInclusive(10, 30) ; 
       defaultCoords[`Circle${i}`]["color"] = 'black'; 
         }
  }
...
if (ballArray[key].radius + ballArray[key].northing > innerHeight|| ballArray[key].northing - ballArray[key].radius < 0)
    ...
    for (i in ballArray) {
        ...
        if ( i>key &&  (x <= ballArray[key].radius || x<ballArray[i].radius) && (x >! ballArray[key].radius || x>!ballArray[i].radius)) {
        
        ballArray[key].color = 'red'; ballArray[i].color = 'green';
